...
const toolInstruction = (
  ...
  if (names.includes('video_runtime_prepare')) {
    guidance.push(
      'For a new Remotion composition, keep source under .sugarcode/video and call video_runtime_prepare instead of installing Remotion in the project. The managed runtime is shared and version-locked; repeated preparation reuses its cache.',
    );
  }
  if (names.includes('video_render')) {
    guidance.push(
      'Use video_render for Remotion sample and final renders. Render a distinct sample before the final output, preserve concurrency one, and use the returned finalDirective unchanged after success so the final video has a quick preview card.',
    );
  }
