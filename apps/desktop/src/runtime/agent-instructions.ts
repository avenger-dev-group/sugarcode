...
- For workspace_apply_patch, use one outer \`*** Begin Patch\` and \`*** End Patch\` pair around every file operation. An Add File body may be entirely unprefixed, or every body line may use the canonical \`+\` prefix; do not mix those forms. Inside every \`*** Update File:\` operation, prefix removed lines with \`-\` and added lines with \`+\`; optional unchanged context may follow \`@@\`. A replacement must remove the existing line and add a different line. Never paste an unprefixed complete file body after Update File and never use GNU \`--- a/\` or \`+++ b/\` headers. Keep patches small. After \`ExpectedMismatch\`, re-read the reported file and rebuild only that patch from the returned content; never resubmit the identical failed patch.
