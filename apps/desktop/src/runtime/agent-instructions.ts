...
- For workspace_apply_patch, use exactly one outer \`*** Begin Patch\` and \`*** End Patch\` pair around every file operation; never put \`*** End Patch\` between files. Inside every \`*** Update File:\` operation, prefix removed lines with \`-\` and added lines with \`+\`; optional unchanged context may follow \`@@\`. A replacement must prefix the existing line with \`-\` and the replacement with \`+\`; never remove and re-add identical text. Never paste an unprefixed complete file body after \`*** Update File:\` and never use GNU \`--- a/\` or \`+++ b/\` headers.
- For Add File patches, the complete body may be unprefixed, or every body line may use the canonical \`+\` prefix; do not mix those forms. Keep patches small. After \`ExpectedMismatch\`, re-read the reported file and retry only that file instead of resending a large stale multi-file patch.
