...
import { ANALYZE_VIDEO_TOOL_NAME } from '../media/video/analysis.ts';
...
export const toolProgressSummary = (
  ...
  if (chinese) {
    switch (toolName) {
      ...
      case 'shell_exec':
        return '正在运行项目命令。';
      case 'video_runtime_prepare':
        return '正在准备 SugarCode 托管视频运行环境。';
      case 'video_render': {
        const output = boundedProgressValue(argumentsValue.outputPath);
        return output
          ? `正在单并发渲染 ${pathBasename(output)}。`
          : '正在单并发渲染视频。';
      }
      case 'video_voiceover':
        return '正在使用本地系统语音生成配音。';
      case 'video_audio_mix': {
        const output = boundedProgressValue(argumentsValue.outputPath);
        return output
          ? `正在混音并生成 ${pathBasename(output)}。`
          : '正在为视频混入音轨。';
      }
      case 'browser': {
  ...
  switch (toolName) {
    ...
    case 'video_runtime_prepare':
      return 'Preparing SugarCode’s managed video runtime.';
    case 'video_render': {
      const output = boundedProgressValue(argumentsValue.outputPath);
      return output
        ? `Rendering ${pathBasename(output)} with one browser worker.`
        : 'Rendering the video with one browser worker.';
    }
    case 'video_voiceover':
      return 'Generating narration with the local system voice.';
    case 'video_audio_mix': {
      const output = boundedProgressValue(argumentsValue.outputPath);
      return output
        ? `Mixing audio into ${pathBasename(output)}.`
        : 'Mixing audio into the video.';
    }
    case 'browser': {
      const action = boundedProgressValue(argumentsValue.action);
      const labels: Readonly<Record<string, string>> = {
...
export const toolResultSummary = (
  ...
  if (
    ...
    if (result.ok !== true) {
      return chinese
        ? '视频制作步骤未完成，请查看错误详情。'
        : 'The video production step did not complete; see the error details.';
    }
    const labels: Readonly<Record<string, readonly [string, string]>> = {
      video_runtime_prepare: ['托管视频运行环境已就绪。', 'The managed video runtime is ready.'],
      video_render: ['视频已渲染并完成媒体检查。', 'The video was rendered and media checks completed.'],
      video_voiceover: ['本地配音音轨已生成。', 'The local narration track was generated.'],
      video_audio_mix: ['音轨已混入，最终视频检查完成。', 'Audio was mixed and the final video check completed.'],
    };
    const label = labels[toolName];
    return label?.[chinese ? 0 : 1];
  }
