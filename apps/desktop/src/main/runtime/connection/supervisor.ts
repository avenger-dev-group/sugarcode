...
export class RuntimeSupervisor {
  ...
  private publishLifecycle = (
    ...
    for (const listener of this.lifecycleListeners) {
      try {
        ...
        this.reportListenerError(error, 'lifecycle');
      }
    }
  };
  ...
  private emit = (event: RuntimeEvent): void => {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        this.reportListenerError(error, 'runtimeEvent');
      }
    }
  };
  ...
  private reportListenerError = (
    ...
    if (this.options.onListenerError) {
      try {
        this.options.onListenerError(error, source);
        return;
      } catch {
        // Diagnostics must not prevent delivery to the remaining subscribers.
      }
    }
    ...
    process.stderr.write(
      `[SugarCode runtime ${source} listener failed] ${message}\n`,
    );
  };
}
