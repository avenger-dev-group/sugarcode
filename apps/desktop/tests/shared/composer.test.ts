...
test('Figma URLs remain message content and the Figma aggregate is an application', () => {
  const url =
  ...
  assert.equal(normalizeComposerContentLinks(`查看\n@${url}`), `查看\n${url}`);
