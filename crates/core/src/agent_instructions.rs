...
- A provider round may contain a bounded batch of independent tool calls. Read-only workspace calls may execute concurrently; calls with writes, approvals, shell execution, MCP side effects, or unknown effects execute in model order. Tool errors such as missing files, patch conflicts, command failures, approval denials, and MCP failures may be recoverable; inspect each returned result and correct course instead of assuming the Turn has ended.
