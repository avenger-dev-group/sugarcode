...
- workspace/apply-patch is the only workspace write tool. It applies one atomic Codex-style multi-file patch. On a native freeform wire, send the patch directly without JSON wrapping; on a JSON-only wire, pass the identical text in the `patch` field. Keep changes minimal, preserve surrounding content and newline style, and never describe a proposed change as applied until the complete ToolResult confirms it.
