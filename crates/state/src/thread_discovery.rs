...
impl ThreadDiscoveryProjection {
    ...
    pub(crate) fn record_thread_created(
        ...
        let result = (|| {
            ...
            transaction
                .execute(
                    "INSERT INTO threads (
                        thread_id, thread_order_key, last_rollout_sequence
                    ) VALUES (?1, ?2, 1)",
    ...
    fn rebuild(
        ...
        #[cfg(not(unix))]
        remove_regular_file_if_present(&self.database_path)?;
...
fn build_database(
        ...
        .map_err(|error| sqlite_error(path, "rebuild", error))?;
    {
        ...
        for snapshot in threads.values() {
            let record_sequence = snapshot.turns.len().checked_add(1).ok_or_else(|| {
                RolloutError::Projection(ProjectionDiagnostic {
                    path: path.to_path_buf(),
                    operation: "rebuild",
                    kind: "limitExceeded",
...
fn validate_projection(
    ...
    let mut statement = connection.prepare(
        "SELECT thread_id, thread_order_key, last_rollout_sequence
    ...
    while let Some(row) = rows.next()? {
        ...
        if order_key != thread_order_key(&snapshot.id).map_err(|_| rusqlite::Error::InvalidQuery)?
            || record_sequence != snapshot.turns.len() as i64 + 1
        {
            return Err(rusqlite::Error::InvalidQuery);
        }
        count += 1;
